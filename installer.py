...
class InstallerEngine:
    ...
    def install(self, setup):
        ...
        try:
            ...
            self.do_run_in_chroot("useradd -s %s -c \'%s\' -G sudo,audio,netdev -m %s" % ("/bin/bash", setup.real_name, setup.username))
            ...
            if(setup.grub_device is not None):
                self.update_progress(pulse=True, total=our_total, current=our_current, message=_("Installing bootloader, please wait!"))
    ...
    def do_configure_grub(self, our_total, our_current):
        self.update_progress(pulse=True, total=our_total, current=our_current, message=_("Configuring bootloader, please be patient!"))
    ...
    def do_check_grub(self, our_total, our_current):
        ...
        if os.path.exists("/target/boot/grub/grub.cfg"):
            ...
            for line in grubfh:
                line = line.rstrip("\r\n")
                if("desktop-grub.png" in line):
                    found_theme = True
                    print " --> Found Grub theme: %s " % line
                if ("menuentry" in line and "towo" in line):
                    found_entry = True
